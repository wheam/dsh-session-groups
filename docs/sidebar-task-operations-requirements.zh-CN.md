...
- 状态：已实现；恢复归档与永久删除等待 DSH Runtime 提供安全 API
- 调研与需求基线：2026-08-23
...
9. 更新 README，并完成 P1/P2 的置顶、拖拽、归档管理、Activity 与批量操作。

本文件既记录已确认的产品需求和范围，也作为本轮实现与验收基线。受 DSH `0.1.1-rc.2` 能力限制，归档恢复和永久删除只展示明确的不可用状态，不直接写底层存储；Scheduled 后端与跨设备偏好同步仍属于非目标。
