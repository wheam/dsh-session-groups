...
| 来源上下文 | 从哪里发起、在哪个聊天中继续 | Provider assignment 的 `source`、`id`、`title`、`kind` | `source + id` | `未标注来源` |
...
未提供 assignment 的 Session 在来源视图进入 `未标注来源`，不能武断断言一定由本地 UI 创建。这类 Session 直接列在该来源桶下，不伪造具体聊天层级。项目视图把缺少来源视为默认状态，不在每个 Session 行重复展示；只有明确的渠道来源才显示行内来源标识。未知 `kind` 原样保留内部语义但不展示未经产品定义的枚举；不得根据标题猜测聊天类型。

### 5.4 两种浏览视图

侧栏提供持久化的顶层视图切换：

- `按项目`：默认视图。一级按真实 Workspace、目录项目和 `无项目` 组织；有明确渠道 assignment 的 Session 行显示紧凑来源标识，例如“飞书 · Curio 作战室”，缺少来源时不重复展示默认文案。
- `按来源`：一级按飞书、Slack、Telegram、`未标注来源` 组织，二级按具体群聊、话题或私聊组织；Session 行显示项目或目录标识，例如“dsh-session-groups”。
...
搜索结果、Activity 和归档管理应展示项目；存在明确渠道来源时再同时展示来源，不能用重复的缺省文案制造无效信息。
...
“待我处理”“失败”“运行中”“刚完成”是按需出现的快捷筛选，不复制或改变 Session 的项目或来源属性。默认不为全零状态长期占据一行；状态数量按当前可见视图聚合，切换视图不会改变跨 Session 的总计。
...
| 运行中 | `running` 或存在运行中 Job | 蓝色旋转 loading | Agent 或后台任务正在工作 |
| 未读（已完成） | `completed` | 蓝色未读圆点与加粗标题 | 未选中期间完成，用户尚未打开查看；打开后恢复已读 |
...
- 项目视图按真实 Workspace、目录项目和 `无项目` 分组；只有明确的渠道来源才在 Session 行显示来源渠道和具体聊天。
...
- 没有 Provider assignment 的 Session 在来源视图进入 `未标注来源`，项目视图不重复显示缺省来源；没有可解析项目上下文的 Session 进入 `无项目`，两种缺失状态互不混淆。
...
- 搜索栏下方按需提供“待我处理”“失败”“运行中”“刚完成”“全部”快捷筛选；非零状态或当前已选状态才常驻，全部为零且未筛选时隐藏整行。
- 快捷筛选显示跨分组数量。
...
- Session 行在 hover 或键盘聚焦时同时提供独立的快捷归档按钮和三点菜单；鼠标移出且菜单关闭后，两者都应消失。
...
- Provider 来源接口失败：项目视图继续按 Workspace/目录展示 Session；保留最近成功 assignment 作为来源标识，完全没有可用 assignment 时在来源视图归入 `未标注来源`，项目视图省略来源副标题。
...
- 每行只保留一个主状态，减少视觉噪声。
- 行内常驻信息优先级为：状态、标题、另一维度的紧凑标识；更新时间和完整上下文收进 tooltip 或详情菜单，不隐藏标题。
- 项目、来源渠道和具体聊天必须使用不同层级或标签语义，不能只靠相似文件夹图标区分。
- 来源一级标题与具体聊天二级标题必须通过字号、字重和缩进形成清晰层级；交互控件统一继承应用字体。
- 项目标题与 Session 主标题使用清晰但克制的中等字重，并尽量落在同一文字基线；不用永久树状竖线表达默认项目层级。
- 选中背景应横向覆盖当前导航列；空 Workspace 只保留项目标题和数量，不增加“暂无会话”占位行。
- 默认隐藏没有行动价值的全零快捷状态，完整筛选能力继续保留在详细筛选面板。
- 视图切换控件必须显示当前组织轴，来源筛选生效时还要显示筛选范围，避免用户混淆“换视图”和“过滤数据”。
- 归档作为 hover 快捷操作保留；其他低频操作进入三点菜单或上下文菜单。
- 分组总数和状态计数口径必须稳定且可解释。
...
4. `无项目` 与 `未标注来源` 的语义和空状态互不混淆，项目视图不在每行重复显示缺省来源。
...
9. Session 行能展示对侧上下文，并可在 tooltip 或详情菜单查看准确更新时间；辅助信息不会挤压主要标题。
